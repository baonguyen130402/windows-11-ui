...
export function Modal(props: any) {
  ...
  const myClassName = clsx(
    "w-28 h-28 fixed transition-all border-slate-600 backdrop-filter backdrop-blur-2xl bg-gray-800/80 text-slate-400 rounded-[0.25rem] p-2 grid grid-cols-3 grid-rows-3 gap-2",
    {
      "-top-32": isOpen,
      "top-32": !isOpen,
    },
...
export function SystemIconTray() {
  ...
  return (
    ...
    >
      <Modal isOpen={open} />
      {!open && <p className="fixed -top-11 hidden group-hover:block group-hover:animate-fade delay-700 bg-[#1b1f1f] rounded-[0.25rem] p-2 text-xs">
        Show hidden icons
      </p>}
      <Icons.lineAngleUp className="size-3 group-hover:rotate-180 group-hover:duration-300" />
    </div>
  );
}
